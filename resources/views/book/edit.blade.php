                ...
                <table class="table table-borderless m-0">
                    <tr>
                        <td>
                            ...
                            <select name="category_id" id="category_id" class="form-select form-control" aria-label="Default select example" autofocus>
                                ...
                                <option selected value="{{ $category->id }}">{{ $category->nama_kategori }}</option>
                                @else
                                <option {{ ($category->id == $book->category_id)? 'selected' : '' }} value="{{ $category->id }}">{{ $category->nama_kategori }}</option>
                                @endif
                                @endforeach
                            </select>
                            @error('category_id')
                    ...
                    <tr>
                        <td>
                            ...
                            <input type="text" id="nama_buku" name="nama_buku" id="nama_buku" class="form-control w-100 @error('nama_buku') is-invalid @enderror" value="{{ old("nama_buku", $book->nama_buku) }}" placeholder="Nama Kategori Buku" required>
                            @error('nama_buku')
                                <div class="invalid-feedback">
                                {{$message}}
                                </div>
                    ...
                    <tr>
                        <td>
                            ...
                            <input type="text" id="stok_buku" name="stok_buku" id="stok_buku" class="form-control w-100 @error('stok_buku') is-invalid @enderror" value="{{ old("stok_buku", $book->stok_buku) }}" placeholder="Jumlah Buku" autofocus required>
                            @error('stok_buku')
                                <div class="invalid-feedback">
                                {{$message}}
                                </div>
